...
class TournamentStatsView(IView):
    ...
    def draw(self, window: curses.window):
        ...
        headers = ["Язык", "Сложность", "Имя",
                   "Равномерность", "Правильные нажатия"]
        ...
        if not self.stats:
            ...
            disp_stat = list(reversed(self.stats))
            visible_rec = disp_stat[self.scroll_position:self.scroll_position +
                                    max_visible_records]

            for record in visible_rec:
                x_pos = 0
                ...
                window.addstr(y_pos, x_pos,
                              record["name"], curses.color_pair(1))
                x_pos += header_widths[2]

                window.addstr(y_pos, x_pos,
                              f"{record["uniformity_score"]}%",
                              curses.color_pair(2))
                x_pos += header_widths[4]

                window.addstr(y_pos, x_pos,
                              f"{record["correct_keystrokes"]}",
                              curses.color_pair(2))

                y_pos += 1

            total_pages = (len(disp_stat) +
    ...
    def get_max_visible_records(self, window_height):
        ...
        """
        # 7 = 2 (заголовок) + 1 (шапка таблицы) +
        # 1 (разделитель) + 3 (нижняя информация)
