...
import curses
from Views.TextOutput import draw_text_with_wrap
...
class ExerciseView(IView):
    ...
    def draw(self, window: curses.window) -> None:
        ...
        self.index_column = draw_text_with_wrap(window, self.exercise_text, 1, 0)

        window.addstr(self.index_column + 1, 0, "Нажмите любую клавишу, чтобы начать...")

        # Показываем лучший результат, если есть
        if self.best_record:
            window.addstr(self.index_column + 3, 0, "Your best result:", curses.color_pair(3))
            window.addstr(self.index_column + 4, 0, f"WPM: {self.best_record['wpm']:.1f} | Accuracy: {self.best_record['accuracy']:.1f}%",
                        curses.color_pair(3))
            window.addstr(self.index_column + 5, 0, f"Date: {self.best_record['timestamp']}", curses.color_pair(3))
    ...
    def update_display(self, window: curses.window, is_correct: bool,
        ...
        window.addstr(self.index_column + 2, 0, feedback, curses.color_pair(color_pair))
        ...
        window.addstr(self.index_column + 4, 0, f"Correct keystrokes: {correct_keystrokes}")
